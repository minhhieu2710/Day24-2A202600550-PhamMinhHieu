...
'''
    ...
    try:
        ...
        print(f"[WARN] pairwise_judge failed: {e}")
...
if __name__ == "__main__":
    ...
    # Load test set to get ground truths for comparison
    from config import TEST_SET_PATH
    with open(TEST_SET_PATH, encoding="utf-8") as f:
        test_set = json.load(f)
    gt_map = {q_item["id"]: q_item["ground_truth"] for q_item in test_set}

    print("Running judge on the 10 human-labeled questions...")
    judge_results = []
    judge_labels = []
    from openai import OpenAI
    client = OpenAI()
    ...
    for item in human_data:
        ...
        m_ans = item["model_answer"]
        g_truth = gt_map.get(q_id, "")
        
        # Keep pairwise results for swap_and_average/bias report
        res = swap_and_average(q_text, m_ans, g_truth)
        judge_results.append(res)
        
        # Direct correctness classification to match human judgment criteria
        eval_prompt = f"""Bạn là một expert đánh giá câu trả lời của hệ thống RAG so với Ground Truth.
Câu hỏi: {q_text}
Câu trả lời của Model: {m_ans}
Ground Truth: {g_truth}

Hãy đánh giá xem câu trả lời của Model có chính xác và đầy đủ thông tin cốt lõi so với Ground Truth hay không.
Nếu chính xác và đầy đủ (hoặc chấp nhận được, không sai lệch thông tin quan trọng), hãy trả về 1.
Nếu sai lệch thông tin, thiếu thông tin quan trọng hoặc hoàn toàn không chính xác, hãy trả về 0.
Chỉ trả về số 1 hoặc 0 dưới dạng văn bản thuần túy, không giải thích gì thêm."""

        try:
            eval_resp = client.chat.completions.create(
                model=JUDGE_MODEL,
                messages=[
                    {"role": "system", "content": "Chỉ trả về 1 hoặc 0."},
                    {"role": "user", "content": eval_prompt}
                ],
                temperature=0.0
            )
            label = int(eval_resp.choices[0].message.content.strip())
        except Exception:
            label = 1 if res.final_winner in ("A", "tie") else 0
            
        judge_labels.append(label)
        print(f"  Q#{q_id}: judge_label={label} (human={item['human_label']})")
    ...
    # Save Phase B report to reports/judge_results.json
